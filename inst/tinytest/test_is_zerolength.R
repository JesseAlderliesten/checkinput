...
#### Tests ####
for(x in list(NULL, numeric(0), character(0), vector(mode = "list"),
...
# A dataframe with zero rows has a length equal to the number of columns.
expect_silent(expect_false(is_zerolength(x = test_df_num[numeric(0), , drop = FALSE])))
...
rm(test_df_logi, test_mat_logi, test_list_logi,
   test_df_num, test_mat_num, test_list_num,
   test_df_char, test_mat_char, test_list_char, x)
