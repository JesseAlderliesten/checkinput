...
warn_susp_both <- paste0("modified by make.names(x, unique = TRUE) or by",
...
expect_warning(expect_false(
  all_names(x = "X.3", allow_suspicious = FALSE)),
  pattern = paste0(warn_susp_readcsv, "'X.3'"), strict = TRUE, fixed = TRUE)
expect_true(all_names(x = "X.3", allow_suspicious = TRUE))

expect_warning(expect_false(
  all_names(x = "e.3", allow_suspicious = FALSE)),
  pattern = paste0(warn_susp_makenames, "'e.3'"), strict = TRUE, fixed = TRUE)
expect_true(all_names(x = "e.3", allow_suspicious = TRUE))
...
expect_warning(expect_false(
  all_names(x = c(".", "..", "...", "...."))),
  pattern = paste0(warn_dots, ", '..', '...', '....'"), strict = TRUE,
  fixed = TRUE)
...
for(allow_suspicious in false_true) {
  ...
  for(x in list(NA, data.frame(a = "nco"), as.matrix(data.frame(a = "nco")),
                list())) {
    expect_warning(
      all_names(x = x, allow_suspicious = allow_suspicious),
      pattern = "'x' is not a character vector!", strict = TRUE, fixed = TRUE)
  }
}

expect_true(all_names(x = susp_readcsv, allow_suspicious = TRUE))
expect_warning(expect_false(
  all_names(x = susp_readcsv, allow_suspicious = FALSE)),
  pattern = paste0(warn_susp_readcsv, susp_readcsv_quoted),
  strict = TRUE, fixed = TRUE)

expect_true(all_names(x = susp_makenames, allow_suspicious = TRUE))
expect_warning(expect_false(
  all_names(x = susp_makenames, allow_suspicious = FALSE)),
  pattern = paste0(warn_susp_makenames, susp_makenames_quoted),
  strict = TRUE, fixed = TRUE)

expect_true(all_names(x = susp_both, allow_suspicious = TRUE))
expect_warning(expect_false(
  all_names(x = susp_both, allow_suspicious = FALSE)),
  pattern = paste0(warn_susp_both, susp_both_quoted),
  strict = TRUE, fixed = TRUE)

expect_true(all_names(x = susp_vecasnames, allow_suspicious = TRUE))
expect_warning(expect_false(
  all_names(x = susp_vecasnames, allow_suspicious = FALSE)),
...
# Mix
x_mix <- c(valid, susp_readcsv, susp_makenames, susp_vecasnames, invalid)
expect_warning(expect_false(
  all_names(x_mix, allow_suspicious = TRUE)),
  pattern = paste0(warn_syntax, invalid_quoted, use_makenames),
  strict = TRUE, fixed = TRUE)
expect_warning(expect_false(
  all_names(x_mix, allow_suspicious = FALSE)),
...
expect_warning(expect_false(
  all_names(x = c(susp_readcsv, susp_readcsv[c(2, 4)]), allow_suspicious = TRUE)),
  pattern = paste0(warn_dupl, checkinput:::paste_quoted(susp_readcsv[c(2, 4)]),
                   use_makenames),
  strict = TRUE, fixed = TRUE)

expect_warning(expect_false(
  all_names(x = c(susp_readcsv, susp_readcsv[c(2, 4)]), allow_suspicious = FALSE)),
  pattern = paste0(warn_dupl, checkinput:::paste_quoted(susp_readcsv[c(2, 4)]),
...
rm(allow_suspicious, empty_string_quoted, false_true, invalid, invalid_quoted,
