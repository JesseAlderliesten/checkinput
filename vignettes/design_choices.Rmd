...
## Length of x
By default, the check functions starting with `is_` only returns `TRUE` for `x`
of length one (with the obvious exception of `is_zerolength(x)`) and the check
functions starting with `all_` only returns `TRUE`
...
`all_<func>(x)`, where `all_<func>` should be replace with the relevant function
