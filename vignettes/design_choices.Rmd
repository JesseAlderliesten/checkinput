...
## Type of x
The check functions of `checkinput`
...
## Length of x
By default, the check functions starting with `is_` only return `TRUE` for `x`
of length one (with the obvious exception of `is_zerolength(x)`) and the check
functions starting with `all_` return `TRUE`
for `x` of length larger than zero. Set argument `allow_zero` to `TRUE` to let
them also
...
## `NA` and `NaN` in x
By default, the check functions return `FALSE` for `x` containing `NA` or
`NaN`. Set argument `allow_NA` to `TRUE` to let them return `TRUE` for `x`
containing `NA` of
the correct type, and set argument `allow_NaN` in functions like `is_number()`
to `TRUE` to let them return `TRUE` for `x` containing `NaN`.

## Return
The check functions of `checkinput` return either
`TRUE` or `FALSE`. To get a named boolean vector indicating for each
element of `x` if it `TRUE` or `FALSE` according to the check functions, use
`vapply(X = x, FUN.VALUE = logical(1), FUN = all_<func>)` instead of
`all_<func>(x)`, where `all_<func>` should be replaced with the relevant function
name. For example, to check which elements of `x` are valid
...
`allow_underscores = FALSE` to not allow underscores, should be placed behind
argument `x`.
