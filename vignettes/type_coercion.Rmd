...
section `Details` in `help(c)` and `help(typeof)`. For example, numeric `314`
will be coerced to character `"314"` when it is combined in a vector with
...
single type, such that `all_characters(unlist(z))` does **not** check if all
